...
class FilesController extends AbstractController
{
    ...
    #[Route('/api/cours/{idCours}/activities/{idActivities}/files', name: 'api_upload_files', methods: ['POST'])]
    public function uploadFiles(ActivitiesRepository $activitiesRepository, UserRepository $userRepository, Request $request, CoursRepository $coursRepository, EntityManagerInterface $em, SerializerInterface $serializer, UrlGeneratorInterface $urlGenerator)
    {
        $cours = $coursRepository->find($request->attributes->get('idCours'));
        if($coursRepository->isUserFromCours($cours,$userRepository->getUserFromToken()) == false){
            return new JsonResponse(['error' => 'Vous ne faites pas partie de ce cours.'], Response::HTTP_FORBIDDEN);
        ...
        $files->setActivities($activities);
        $files->setSeance($activities->getSeance());
        ...
        return new JsonResponse($jsonFiles, Response::HTTP_CREATED, [], true);
    }
}
